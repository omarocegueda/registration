...
if [ -r $deformationField ]; then
    ...
    exe="ANTS 3 -m  MI[reference/$reference,target/$target,1,32] -t SyN[0.25] -a ${affine} -r Gauss[3,0] -o ${targetbase}_${referencebase} -i 100x100x25 --continue-affine false"
    echo " $exe "
    $exe
fi
